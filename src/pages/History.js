  ...
  return (
    <div className="min-h-screen pb-32">
      <main className="max-w-7xl mx-auto px-6 py-20">
        ...
        <header className="mb-20">
          <div className="flex items-center justify-between">
            ...
            >
              <div className="w-16 h-16 rounded-[2rem] glass flex items-center justify-center shadow-xl shadow-[#9370DB]/10">
                <History className="w-8 h-8 text-[#9370DB]" />
              </div>
              <div>
                <h1 className="text-5xl font-black text-[#1a0b2e] tracking-tight">Hub Archive</h1>
                <p className="text-[#1a0b2e]/40 text-sm font-bold uppercase tracking-[0.2em] mt-2">Chronicles of your orchestrated journeys</p>
              </div>
            </motion.div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 card-3d-wrapper">
            <AnimatePresence>
              {trips.map((trip, i) => (
                ...
                >
                  <div className="absolute -top-12 -right-12 w-40 h-40 bg-[#9370DB]/10 rounded-full blur-[60px] group-hover:bg-[#9370DB]/20 transition-all duration-700" />
                  
                  <div className="flex items-center justify-between mb-10 relative z-10">
                     <span className="text-[10px] font-black text-[#1a0b2e] bg-white/40 px-5 py-2 rounded-full uppercase tracking-[0.2em] border border-[#9370DB]/20 group-hover:bg-[#9370DB] group-hover:text-white transition-all duration-500">
                       {trip.status}
                     </span>
                  ...
                  <h3 className="text-3xl font-black text-[#1a0b2e] mb-8 group-hover:translate-x-2 transition-transform duration-500 relative z-10 leading-tight">
                    {trip.details.from_location} <ArrowRight className="inline w-5 h-5 mx-2 text-[#9370DB]/30" /> {trip.details.destination}
                  </h3>
                  ...
                  <div className="space-y-5 mb-10 relative z-10">
                    ...
                    <div className="flex items-center gap-4 text-sm font-bold text-[#1a0b2e]/40 group-hover:text-[#1a0b2e]/70 transition-colors">
                      <Users className="w-5 h-5 text-[#9370DB]" />
                      {trip.details.num_people} Personnel Synchronized
                    </div>
                  </div>

                  <div className="pt-8 border-t border-[#1a0b2e]/5 flex items-center justify-between group-hover:border-[#9370DB]/20 transition-all duration-500 relative z-10">
                     <div className="flex items-center gap-3">
                        <div className="bg-white/60 p-3 rounded-2xl group-hover:bg-[#9370DB] transition-all duration-500 group-hover:shadow-lg group-hover:shadow-[#9370DB]/20">
                          {trip.details.transport_mode === 'flight' && <Plane className="w-5 h-5 text-[#9370DB] group-hover:text-white" />}
                          {trip.details.transport_mode === 'train' && <Train className="w-5 h-5 text-[#9370DB] group-hover:text-white" />}
                          {trip.details.transport_mode === 'car' && <Car className="w-5 h-5 text-[#9370DB] group-hover:text-white" />}
                        </div>
          ...
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-center py-40 glass rounded-[4rem] border-dashed border-2 border-[#9370DB]/10 relative overflow-hidden"
          >
            <History className="w-24 h-24 text-[#9370DB]/10 mx-auto mb-10" />
            ...
            <motion.button
              ...
              className="mt-12 px-10 py-5 bg-[#9370DB] text-white rounded-full font-black text-sm uppercase tracking-widest shadow-xl shadow-[#9370DB]/20"
            >
              Initiate Deployment
            </motion.button>
          </motion.div>
      ...
      <div className="fixed top-0 left-0 w-full h-full -z-50 pointer-events-none overflow-hidden">
        <div className="absolute top-[20%] right-[10%] w-[600px] h-[600px] bg-[#9370DB]/5 rounded-full blur-[140px] pulse-bg" />
