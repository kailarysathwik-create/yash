...
const TripPlanner = () => {
  ...
  useEffect(() => {
    const fetchTrip = async () => {
      try {
        ...
        toast.error('Failed to synchronize trip data');
        navigate('/dashboard');
      } finally {
        setLoading(false);
      }
  ...
  const handleOrchestrate = async () => {
    ...
    try {
      ...
      toast.success('Hub AI: Orchestration protocol synchronized.');
    } catch (error) {
        ...
        <header className="mb-16">
          <div className="flex items-center justify-between mb-8">
            <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
              ...
              <h1 className="text-5xl font-black text-[#1a0b2e] tracking-tight">Deployment Protocol</h1>
            </motion.div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-12">
               <div className="flex items-center justify-between mb-8">
                  <h2 className="text-4xl font-black text-[#1a0b2e] tracking-tight">Orchestration Manifest</h2>
                  <div className="glass px-6 py-3 rounded-full flex items-center gap-3">
                     <div className="w-3 h-3 bg-purple-500 rounded-full animate-bounce" />
                     <span className="text-[10px] font-black uppercase text-[#1a0b2e] tracking-widest">Final Validation</span>
                  </div>
               </div>
          ...
          {step === 5 && (
            <motion.div key="step5" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              <UPIPayment 
                amount={(selectedTransport?.price || 0) + (selectedStay?.price || 0)} 
                tripId={tripId} 
                onComplete={() => {
