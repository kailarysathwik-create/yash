    ...
    <div className="space-y-12 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-12 card-3d-wrapper">
        {options.map((option, index) => (
          ...
          >
            <div className="absolute -bottom-20 -left-20 w-60 h-60 bg-[#9370DB]/5 rounded-full blur-[80px] group-hover:bg-[#9370DB]/15 transition-all duration-700" />
            
            <div className="md:w-1/3 flex flex-col items-center justify-center p-8 bg-white/60 rounded-[3rem] group-hover:bg-[#9370DB] transition-all duration-700 group-hover:shadow-2xl group-hover:shadow-[#9370DB]/20">
               <Home className="w-16 h-16 text-[#9370DB] group-hover:text-white mb-6 transition-colors" />
               <div className="flex items-center gap-1">
                 {[1, 2, 3, 4, 5].map((s) => (
                   <Star key={s} className="w-3 h-3 fill-[#9370DB] text-[#9370DB] group-hover:fill-white group-hover:text-white" />
                 ))}
               </div>
            </div>
            ...
            <div className="md:w-2/3 flex flex-col justify-between relative z-10">
              <div>
                <div className="flex items-center justify-between mb-6">
                   <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#9370DB]">Premium Node</span>
                ...
                <div className="flex items-center gap-2 text-[#1a0b2e]/40 font-bold text-sm mb-8">
                   <MapPin className="w-4 h-4 text-[#9370DB]" /> {option.description}
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-[#1a0b2e]/5 flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <div className="text-4xl font-black text-[#9370DB]">₹{option.price.toLocaleString()}</div>
                </div>
